...
Bool intersectLine(double lineOne[DRAWING_DIMENSIONS],
		     double lineTwo[DRAWING_DIMENSIONS],
		     double intersection[DRAWING_DIMENSIONS]){
  /*lines given in terms of [0] is slope, [1] is intercept*/
  /*intersection given in terms of [0] is x, [1] is y coord*/
  /*returns false if no intersect*/
  
  if(lineOne[0] == lineTwo[0]){
      return False;
  }
  intersection[0] = (lineTwo[1]-lineOne[1])/(lineOne[0]-lineTwo[0]);
  intersection[1] = intersection[0]*lineOne[0] + lineOne[1];
  return True;  
}
